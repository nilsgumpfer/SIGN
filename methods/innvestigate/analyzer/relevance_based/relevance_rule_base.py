    ...
    def wrap_hook(self, ins, neuron_selection, stop_mapping_at_layers, r_init):
        with tf.GradientTape(persistent=True) as tape:
            ...
            if len(self.layer_next) == 0 or (stop_mapping_at_layers is not None and self.name in stop_mapping_at_layers):
                ...
                # print('ZRule ReplacementLayer init:', Zs, r_init)
...
class StdxEpsilonRule(reverse_map.ReplacementLayer):
    """
    Similar to ZRule.
    The only difference is the addition of a numerical stabilizer term
    epsilon to the decomposition function's denominator.
    ...
    def __init__(self, layer, *args, **kwargs):
        self._stdfactor = kwargs.pop("stdfactor", 0.25)
        bias = kwargs.pop("bias", True)
        self._layer_wo_act = kgraph.copy_layer_wo_activation(layer,
                                                             keep_bias=bias,
                                                             name_template="no_act_%s")
        super(StdxEpsilonRule, self).__init__(layer, *args, **kwargs)
    ...
    def explain_hook(self, ins, reversed_outs, args):
        ...
        '''Source: https://git.tu-berlin.de/gmontavon/lrp-tutorial/-/blob/main/tutorial.ipynb)'''
...
class SIGNRule(reverse_map.ReplacementLayer):
    def __init__(self, layer, *args, **kwargs):
        ...
        self._layer_wo_act = kgraph.copy_layer_wo_activation(layer,
                                                             ...
                                                             name_template="no_act_%s")
        # print(self._layer_wo_act.get_config()["use_bias"])
        super(SIGNRule, self).__init__(layer, *args, **kwargs)
    ...
    def explain_hook(self, ins, reversed_outs, args):
        ...
        signs = np.nan_to_num(ins / np.abs(ins), nan=1.0)
        ret = keras_layers.Multiply()([signs, tmp2])

        return ret


class SIGNmuRule(reverse_map.ReplacementLayer):
    def __init__(self, layer, *args, **kwargs):
        bias = kwargs.pop("bias", True)
        ...
        self._layer_wo_act = kgraph.copy_layer_wo_activation(layer,
                                                             keep_bias=bias,
                                                             name_template="no_act_%s")
        super(SIGNmuRule, self).__init__(layer, *args, **kwargs)
    ...
    def explain_hook(self, ins, reversed_outs, args):
        ...
        # Handle special case: First layer is flatten layer (e.g. dense net)
        if np.iterable(self.mu):
            if len(np.ravel(self.mu)) == np.shape(ins)[-1]: # TODO: check if column- or row-wise ravel is required
                self.mu = np.array([np.ravel(self.mu)])

        fsigns = np.array(ins)
        fsigns[fsigns < self.mu] = -1
        fsigns[fsigns >= self.mu] = 1

        ret = keras_layers.Multiply()([fsigns, tmp2])
...
class BoundedRule(reverse_map.ReplacementLayer):
    ...
    def __init__(self,
        ...
        self._low = kwargs.pop("low", -1)
        self._high = kwargs.pop("high", 1)
    ...
    def explain_hook(self, ins, reversed_outs, args):
        ...
        if len(self.layer_next) > 1:
            ...
            if len(self.input_shape) > 1:
                ...
                ret = [keras_layers.Add()([keras_layers.Multiply()([i, t]) for t in tmp2[idx]]) for idx, i in
                       enumerate(ins)]
            else:
                tmp2 = [tape.gradient(Zs, ins, output_gradients=t) for t in tmp]
                ret = keras_layers.Add()([keras_layers.Multiply()([ins, t]) for t in tmp2])
